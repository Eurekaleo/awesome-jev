...
The core studies appeared between 19 and 23 September 2026, beginning four days after the launch (Figure 1). Relationships overlap: {{stat:rel_commercial_core}} studies evaluate or use hosted Jev, {{stat:rel_independent_core}} build or evaluate independent Jev-like models, and {{stat:rel_downstream_core}} embed typed decisions in a larger system (Table 3). Of the {{stat:jev_querying}} studies that query hosted Jev, {{stat:jev_versioned}} state the version they called (`jev-1.13.0` or `jev-1.13`; one uses `typesafe/jev1.13` through OpenRouter [@arxiv2609_24574]) and {{stat:jev_unversioned}} state none. Every core study is an arXiv preprint; two had a second version by 24 September, and their evidence was read on the first.
...
Consistency is not correctness either. On contract inference, five of Jev's 30 fixed anchors received the same wrong label in all twelve responses across request conditions and repeats [@arxiv2609_27678].
...
Two new studies point the same way. On radiology report factuality, raw sentence-level probabilities had ECE 0.1192, lowered to 0.0077 on a held-out half by isotonic calibration [@arxiv2609_27607]. In population simulation, honest intervals needed a fitted human–model discrepancy: coverage was 0.93 and 0.96 at nominal 0.80 and 0.90, against 0.29 and 0.36 from sampling alone [@arxiv2609_27535].
...
The pattern extends to new domains. On contract inference, Jev was the cheapest and fastest of ten configurations but below all seven hosted LLMs on accuracy (77.38% against 78.96–83.21%) [@arxiv2609_27678]. As a radiology report judge, a one-question configuration beat an open NLI judge but trailed a local 27B RadMatch on clinically significant errors [@arxiv2609_27607]. In population experiments, the Jev kernel passed content-fidelity gates but failed intervention-sensitivity gates until sparse flagship anchors, on 1.7% of states, cut effect error by 41% [@arxiv2609_27535].
...
Newer figures keep the scopes apart: $0.000228 per contract at a 1.24 s median, measured end to end [@arxiv2609_27678]; judgment calls priced at under three cents per hundred radiology report pairs, excluding local statement extraction [@arxiv2609_27607]; $0.0227 per thousand predictions at a 213 ms median [@arxiv2609_27535]; and a 0.422 s median action response inside a StarCraft II controller whose planning calls take 27.6 s and 96% of the model cost [@arxiv2609_27331].
